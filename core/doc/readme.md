...
- Workbook-template Basics parser now treats the first fully empty kit-component row after parsed data as end-of-table, preventing cascaded `missing-required-field` (`Assay key is required`) diagnostics from trailing blank rows while logging a debug trace for the detected table boundary.
