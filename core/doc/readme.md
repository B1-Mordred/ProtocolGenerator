...
- Default-ruleset generation now normalizes analyte-to-assay linkage before preview/export: analyte `assay_key` values are resolved against assay identity aliases when possible, and unresolved analyte-only assay references are synthesized into deterministic assay groups so all analytes are materialized in generated `Analytes.xml`.
...
- Analytes XML export now omits any `<Assay>` entries that do not have a non-empty `Assay Abbreviation`, so analytes are only written for assays with assigned abbreviations.
