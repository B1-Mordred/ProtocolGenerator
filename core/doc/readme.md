...
- Added top-level **AddOn Data Entry** and **Data Review** menu workflows: users can choose `Enter Data Manually` or `Import Excel File`; manual mode now provides tabbed Basics/Kit Components/Analytes/Sample Prep/Dilutions entry with on-change autosave snapshots, and Data Review opens the Import Review editor for post-import/manual editing. Basics manual entry now focuses on kit/add-on metadata (Method ID/Version/Display Name removed; `Kit Name` added), and workbook-template import tolerates case/spacing variants in expected sheet names.
