...
- Addon XML generation pipeline under `src/addon_generator/generators/`, `src/addon_generator/serialization/`, and `src/addon_generator/validation/` emits schema-compliant analytes at `AddOn -> Assays -> Assay -> Analytes -> Analyte -> AnalyteUnits -> AnalyteUnit` (using `Analyte/AssayRef` and `AnalyteUnit/AnalyteRef`), maps XML `MethodId` from Basics `AddOn Product Number`, and intentionally leaves XML `MethodVersion` blank for current downstream compatibility.
