...
- Changed Field Mapping conflict handling for locked targets so active-template ownership is exclusive (`Analytes.xml:MethodId`, `Analytes.xml:MethodVersion`, `Analytes.xml:Assays[].Assay.Name`, `Analytes.xml:Assays[].Assay.AssayInformationType`, `ProtocolFile.json:MethodInformation.Id`, `ProtocolFile.json:MethodInformation.Version`, `ProtocolFile.json:AssayInformation[].Type`); conflicting rows are ignored (not last-write-wins), ownership ignores are reported deterministically, and preview/export parity is covered by regression tests.
...
- Fixed workbook-template Basics/analyte linkage key selection to prefer `Assay Abbreviation` as the canonical derived assay key (when `Assay Key` is blank), while keeping `Parameter Set`/component aliases for robust analyte matching and deterministic fallback to `Parameter Set Number` when abbreviations are missing.
...
- Fixed Excel-import manual-entry assay backfill to use raw imported assay rows (while keeping merged-state recomputation for conflict/global state), preserving repeated parameter-set kit-component rows that may be collapsed by merge deduplication and keeping draft save/restore row counts stable for those fixtures.
...
- Changed manual-entry Basics and snapshot mapping to include `addon_version` (`AddOn Version`) and route it to method `method_version` during shell sync/import backfill and draft recovery payload conversion.
